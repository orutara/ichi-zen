<script setup>

</script>

<template>
  <button
    class="bg-twitter w-11 h-11 rounded-full flex justify-center align-baseline pt-2 fixed bottom-3 right-3 lg:hidden">
    <svg version="1.1" id="_x32_" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px"
      y="0px" width="512px" height="512px" viewBox="0 0 512 512" style="width: 23px; height: 27px; opacity: 1;"
      xml:space="preserve">
      <g>
        <path class="st0" d="M473.818,0.003c0,0-18.578,28.297-139.938,70.172c-123.719,42.688-199.875,216.406-199.875,216.406
            c-18.344,35.578-90.813,183.453-90.813,183.453c-19.953,38.172,16.625,60.734,38.063,21.313
            c41.156-75.703,67.688-144.875,130.25-146.844c91.219-2.875,153.609-84.109,133.984-80.359
            c-25.844,11.484-82.781,0.875-49.234-4.391c80.531-6.594,130.125-68.297,113.969-72.5c-28.563,11.219-55.172,0.578-60.391-2.656
            C491.959,166.987,473.818,0.003,473.818,0.003z" style="fill: rgb(255, 255, 255);">
        </path>
      </g>
    </svg>
  </button>
  <!-- / TweetButtonSp -->
</template>